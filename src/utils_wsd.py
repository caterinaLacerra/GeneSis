...
class WSDSubstitutesInstance(NamedTuple):
    annotated_token: AnnotatedToken
...
def read_from_raganato_with_substitutes(
        xml_path: str,
        substitutes_path: str,
        key_path: Optional[str] = None,
        instance_transform: Optional[Callable[[WSDInstance], WSDInstance]] = None,
) -> Iterable[Tuple[str, str, List[WSDInstance]]]:
    def read_by_text_iter(xml_path: str):
    ...
    with open(substitutes_path) as f:
        for line in f:
            line = line.strip()
            key, *substitutes = line.split('\t')
            substitutes_dict[key] = substitutes
    ...
    for document_id, sentence_id, element in read_by_text_iter(xml_path):
        ...
        wsd_instance = WSDSubstitutesInstance(
            annotated_token=annotated_token,
            labels=None
            if element.tag == "wf" or element.attrib["id"] not in mapping
            else mapping[element.attrib["id"]],
            instance_id=None if element.tag == "wf" else element.attrib["id"],
            substitutes=None
            if element.tag == "wf" or element.attrib["id"] not in substitutes_dict
            else substitutes_dict[element.attrib["id"]]
        )
...
def read_from_raganato_with_substitutes_to_load(
        xml_path: str,
        substitutes_path: str,
        key_path: Optional[str] = None,
        instance_transform: Optional[Callable[[WSDInstance], WSDInstance]] = None,
) -> Iterable[Tuple[str, str, List[WSDInstance]]]:
    def read_by_text_iter(xml_path: str, substitutes: Dict[str, List[str]], mapping: Dict[str, List[str]]):
        ...
        for event, elem in it:
            ...
            if event == "end":
                ...
                document_id = sentence_id.split('.')[0]
                if sentence != []:
                    yield document_id, sentence_id, sentence
                else:
                    yield None
                    sentence = []
                ...
                for element in elem:
                    ...
                    wsd_instance = WSDSubstitutesInstance(
                        ...
                        instance_id=None if element.tag == "wf" else element.attrib["id"],
                        substitutes=None if element.tag == "wf" or element.attrib["id"] not in substitutes else
                        substitutes[element.attrib["id"]]
                    )
    ...
    substitutes = {}
    for line in open(substitutes_path):
        line = line.strip()
        wsd_instance, *candidates = line.split("\t")
        substitutes[wsd_instance] = candidates

    for (document_id, sentence_id, sentence) in read_by_text_iter(xml_path, substitutes, mapping):
...
def expand_raganato_path_with_substitutes(path: str) -> Tuple[str, str, str]:
    return f"{path}.data.xml", f"{path}.substitutes.txt", f"{path}.gold.key.txt"
...
def get_gloss_from_sensekey(sensekey: str) -> str:
