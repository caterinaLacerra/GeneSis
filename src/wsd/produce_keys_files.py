...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--input_path', required=True, help='generation output path, as generated with test_on_wsd.py'
                                                            ' script.')
        ...
        for batch in yield_batch(args.input_path, separator='#########\n'):
            ...
            if len(candidates) > 0:
                ...
                for c in candidates:
                    cand = c.split(': ')[0]
                    if not ' ' in cand:
                        to_write.append(cand)

                substitutes.write('\t'.join(to_write) + '\n')
