...
def get_senses(input_path: str, gold_path: str):
    ...
    print(f"avg senses per target: {sum([len(senses_per_target[t]) for t in senses_per_target])/len(senses_per_target)}")

    for pos in targets_per_pos:
        print(f"avg senses per {pos}: {sum([len(list_senses) for list_senses in targets_per_pos[pos]])/len(targets_per_pos[pos])}")
...
if __name__ == '__main__':
    ...
    '''recover_original_datasets(input_paths=["data/translated_datasets/multimirror.de.clean_targets_instances_train.tsv",
                                           "data/translated_datasets/multimirror.wngt.de.clean_targets_train.tsv"],
                              suffixes=["semcor", "wngt"],
                              input_gold=["/media/ssd1/caterina/multimirror/semcor-translated-de-ours-loosebn/dataset/dataset.gold.key.txt",
                                          "/media/ssd1/caterina/multimirror/wngt-translated-de-ours-loosebn/dataset/dataset.gold.key.txt"],
                              merged_path="data/translated_datasets/de_merge.tsv",
                              new_gold_path="/media/ssd1/caterina/multimirror/merged_datasets.de.gold.key.txt",
                              new_output_path="data/translated_datasets/multimirror.semcor_wngt.de.clean_targets_train.tsv")'''