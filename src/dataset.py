...
class MBartDataset(IterableDataset):

    def __init__(self, input_path: str, bart_model: str, src_lang: str, max_tokens_per_batch: int,
                 chunk_size: int = 4000, gold_path: str = None,
                 input_dataset_path: str = None) -> None:

        super(MBartDataset, self).__init__()

        self.input_path = input_path
        self.tokenizer = MBart50TokenizerFast.from_pretrained(bart_model)
    ...
    def encode(self, source: str, target: Optional[str] = None) -> Tuple[List[int], Optional[List[int]]]:
        ...
        model_inputs = self.tokenizer(source, return_tensors='pt')

        if target:
            labels = self.tokenizer(target, return_tensors='pt')
            model_inputs["labels"] = labels["input_ids"]
            return model_inputs['input_ids'][0], model_inputs['labels'][0]
